...
request.prototype.display = function(file){
    ...
    try{
        ...
        expires.setTime(expires.getTime() + (this.vhost.expires||harbors.config.server.expires-0));
        this.res
            .setHeader('Cache-Control','max-age=' + this.vhost.expires||harbors.config.server.expires);
        this.res
        ...
        if(this.vhost.gzip||harbors.config.server.gzip!='false'){
            var raw = fs.createReadStream(file);
            var acceptEncoding =
                this.req
                    .headers['accept-encoding'] || "";
            var matched =
                new RegExp('('+(this.vhost.gzip_file||harbors.config.server.gzip_file)+')')
                    .test(file);
...
request.prototype.send = function(string){
    var self = this;
    string.length && this.res.setHeader('Content-Length',string.length);
