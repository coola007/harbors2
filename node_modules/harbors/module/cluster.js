...
exports.server = function(req,res){
    ...
    _m_php.response(request);
    return undefined;

    //检测静态文件存在
    //console.log(request.vhost.dir + request.path.url);
    var stM = $fs.exists(request.vhost.dir + request.path.url);
    //console.log(stM);
    if(stM){
        harbors.debug('cluser.request is static [Module module/cluster.js]');
        request.display(request.path.url);
        return undefined;
    }

    //检测控制器文件存在
    //console.log(request.vhost.dir + request.path.url + request.vhost.module);
    var dyM = $fs.exists(request.vhost.dir + request.path.url + request.vhost.module);
    //console.log(dyM);
    if(dyM){
